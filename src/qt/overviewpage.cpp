...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeQuantisAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeQuantisAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAmountAndRounds = strAnonymizeQuantisAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeQuantisAmount*COIN) nMaxToAnonymize = nAnonymizeQuantisAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeQuantisAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeQuantisAmount));
        strAmountAndRounds = strAnonymizeQuantisAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
